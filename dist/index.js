...
function renderLegacyTagList(tags, ctx) {
  return /* @__PURE__ */ u2("ul", { class: "tags", children: tags.map((tag) => {
    const href = resolveRelative2(ctx.slug, `tags/${tag}`);
    return /* @__PURE__ */ u2("li", { children: /* @__PURE__ */ u2("a", { href, class: "internal tag-link", children: tag }) });
  }) });
}
var NoteProperties_default = ((opts) => {
  ...
  const Component = (props) => {
    ...
    const tags = Array.isArray(properties.tags) && properties.tags.length > 0 ? properties.tags : null;
    const legacyTag = noteProps.legacyTag ?? "never";
    const useLegacy = tags !== null && (legacyTag === "always" || legacyTag === "only" && entries.length === 1 && "tags" in properties);
    if (useLegacy) {
